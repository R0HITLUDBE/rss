...
SELECT id, created_at, updated_at, title, description, published_at, url, feed_id FROM posts ORDER BY created_at ASC LIMIT 10
`

func (q *Queries) GetLatestPost(ctx context.Context) ([]Post, error) {
	rows, err := q.db.QueryContext(ctx, getLatestPost)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
